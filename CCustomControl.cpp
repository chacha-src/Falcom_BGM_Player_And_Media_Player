...
static void CCC_SetDibAlphaGlassRect(void* pBits, int dibW, int dibH, const RECT& rc, COLORREF clrKey, BYTE alpha)
{
    ...
    const UINT32 a = ((UINT32)alpha) << 24;
...
static void CCC_SetDibAlphaGlass(void* pBits, int w, int h, COLORREF clrKey, BYTE contentAlpha)
{
    if (!pBits || w <= 0 || h <= 0) return;
    const UINT32 key = CCC_RgbMask(clrKey);
    const UINT32 a = ((UINT32)contentAlpha) << 24;
    UINT32* px = (UINT32*)pBits;
    const int n = w * h;
    for (int i = 0; i < n; ++i)
    {
...
static void CCC_PrimeParentBlurUnderChild(HWND hChild)
{
    if (!hChild || !::IsWindow(hChild)) return;
    if (!CCC_IsAeroEnabled() || !CCC_IsWin11() || !CCC_IsBlurDialogChild(hChild)) return;
    HWND hParent = ::GetParent(hChild);
    if (!hParent || !::IsWindow(hParent)) return;
    RECT rc = {};
    ::GetWindowRect(hChild, &rc);
    ::MapWindowPoints(HWND_DESKTOP, hParent, (POINT*)&rc, 2);
    HDC hdcParent = ::GetDCEx(hParent, NULL, DCX_CACHE | DCX_CLIPSIBLINGS);
    if (!hdcParent) return;
    CCC_ClearRectChroma(hdcParent, rc, CCC_AERO_CHROMA_KEY);
...
static void CCC_BlitDialogBackdropUnderPopup(HWND hwndCombo, HWND hwndPopup, HDC hdcDest, int destW, int destH)
{
    ...
    if (!::PrintWindow(hDlg, dcCap.GetSafeHdc(), PW_CLIENTONLY | 0x2))
        ::SendMessage(hDlg, WM_PRINT, (WPARAM)dcCap.GetSafeHdc(), PRF_CLIENT | PRF_CHILDREN | PRF_ERASEBKGND);
...
void CCustomListBox::PaintListBoxGlassBackgrounds(CDC& dc, const CRect& rcClient, BOOL bChromaSolid)
{
    ...
    if (GetItemRect(nLast, &rLast) && rLast.bottom < rcClient.bottom)
    {
        ...
        if (bChromaSolid)
            ...
            FillRectAlpha(&dc, fill, COLOR_LIST_BG, (BYTE)200);
    }
}
...
void CCustomListBox::PaintGlassClient(CDC& dc)
{
    ...
    if (hGlassDib)
    {
        ...
        if (nLast >= nFirst && GetItemRect(nLast, &rLast) && rLast.bottom < rcC.bottom)
        {
            ...
            CCC_SetDibAlphaGlassRect(pGlassBits, w, h, fill, CCC_AERO_CHROMA_KEY, (BYTE)200);
        }
    ...
    if (!hdcBuf || !hBP || !hGlassDib)
    {
        ...
        CCC_BlitChromaGlass(dc.GetSafeHdc(), 0, 0, w, h, memFb.GetSafeHdc(), 0, 0, CCC_AERO_CHROMA_KEY, (BYTE)AERO_ALPHA_SEMI);
...
void CCustomComboBox::PaintGlassClient(CDC& dc)
{
#if CCUSTOM_AERO_SUPPORT
    CRect rcC;
    GetClientRect(&rcC);
    ...
    if (hGlassDib)
    {
        obGlass = ::SelectObject(memGlass.GetSafeHdc(), hGlassDib);
        memGlass.FillSolidRect(0, 0, w, h, CCC_AERO_CHROMA_KEY);
        memGlass.FillSolidRect(&rcC, COLOR_COMBO_BG);
        RECT rr = rcC;
        CCC_SetDibAlphaGlassRect(pGlassBits, w, h, rr, CCC_AERO_CHROMA_KEY, (BYTE)AERO_ALPHA_SEMI);
        CCC_SetDibChromaTransparent(pGlassBits, w, h, CCC_AERO_CHROMA_KEY);
    }
    ...
    if (!hdcBuf || !hBP || !hGlassDib)
    {
        ...
        CBitmap* obFb = memFb.SelectObject(&bmpFb);
        memFb.FillSolidRect(0, 0, w, h, CCC_AERO_CHROMA_KEY);
        memFb.FillSolidRect(&rcC, COLOR_COMBO_BG);
        CCC_BlitChromaGlass(dc.GetSafeHdc(), 0, 0, w, h, memFb.GetSafeHdc(), 0, 0, CCC_AERO_CHROMA_KEY, (BYTE)AERO_ALPHA_SEMI);
        memFb.SelectObject(obFb);
        PaintComboClosedForeground(dc, rcC);
        return;
    }
...
void CCustomComboBox::PaintComboDropGlassBackgrounds(CDC& dc, HWND hwndList, const CRect& rcClient, BOOL bChromaSolid)
{
    ...
    if (::SendMessage(hwndList, LB_GETITEMRECT, nLast, (LPARAM)&rrLast) && rrLast.bottom < rcClient.bottom)
    {
        ...
        const BYTE fillAlpha = bNoScroll ? (BYTE)190 : (BYTE)AERO_ALPHA_SEMI;
        if (bChromaSolid)
...
void CCustomComboBox::DrawItem(LPDRAWITEMSTRUCT lp)
{
    ...
    const BYTE alpha = bS ? (BYTE)180 : (bD ? (BYTE)200 : (BYTE)AERO_ALPHA_SEMI);

...
BEGIN_MESSAGE_MAP(CMpCommandPaletteDlg, CCustomBlurDialogExBase)
	ON_EN_CHANGE(IDC_CMDPAL_FILTER, OnFilterChange)
	ON_BN_CLICKED(IDC_CMDPAL_RUN, OnBnClickedRun)
	ON_BN_CLICKED(IDC_CMDPAL_CLOSE, OnBnClickedClose)
	ON_NOTIFY(NM_DBLCLK, IDC_CMDPAL_LIST, OnDblclkList)
	ON_WM_CLOSE()
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()

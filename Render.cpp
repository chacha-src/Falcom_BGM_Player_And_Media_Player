...
BOOL CRender::OnInitDialog()
{
	...
	m_tooltip.AddTool(GetDlgItem(IDC_CHECK47), LL14(L"mp3のデコーダをオリジナルのデコーダを使わずに、独自で使ったデコーダを使う。\nエラーなどで演奏できないときにチェック入れて下さい。\nまた独自で正常にならない時ははずして下さい。", L"Use custom mp3 decoder instead of original.\nCheck if playback fails.\nUncheck if custom causes issues.", L"Utiliser le decodeur mp3 personnalise.\nCochez si la lecture echoue.\nDecochez s'il pose probleme.", L"Usa decodificatore mp3 personalizzato.\nSpunta se la riproduzione fallisce.\nTogli se causa problemi.", L"Usar decodificador mp3 personalizado.\nMarca si falla la reproduccion.\nDesmarca si causa problemas.", L"mp3를 내장이 아닌 자체 디코더로 재생.\n오류 시 체크, 문제 있으면 해제.", L"使用自定义 mp3 解码器。\n播放失败时勾选；若异常请取消。", L"استخدام فك mp3 مخصص.\nحدّد عند فشل التشغيل.\nألغِ إن سبب مشاكل.", L"Использовать свой mp3-декодер.\nОтметьте при сбое воспроизведения.\nСнимите, если мешает.", L"Eigenen mp3-Decoder nutzen.\nAnhaken bei Fehlern.\nAb wenn Probleme.", L"Usar decodificador mp3 proprio.\nMarque se falhar.\nDesmarque se der problema.", L"Eigen mp3-decoder gebruiken.\nAanvinken bij fouten.\nUit bij problemen.", L"Uzyj wlasnego dekodera mp3.\nZaznacz przy bledach.\nOdznacz gdy szkodzi.", L"Ozel mp3 kod cozucu kullan.\nCalma hatasinda isaretle.\nSorun cikarirsa kaldir."));
	m_tooltip.AddTool(GetDlgItem(IDC_CHECK48), LL14(L"複数音声のある動画を再生する時に、再生前に\n音声ストリームの選択画面を表示します。\n通常ストリーム1がメインとして使われ、ストリーム2以降はコメンタリや英語音声などに使われています。", L"Show audio stream selection before playing multi-audio video.\nStream 1 is usually main; 2+ for commentary/English etc.", L"Afficher le choix de piste audio avant lecture video multi-pistes.\nPiste 1 = principale ; 2+ = commentaire/anglais etc.", L"Mostra scelta traccia audio prima di video multi-audio.\nTraccia 1 = principale; 2+ = commento/inglese ecc.", L"Mostrar seleccion de pista antes de video multi-audio.\nPista 1 = principal; 2+ = comentario/ingles etc.", L"다중 음성 동영상 재생 전\n음성 스트림 선택 화면 표시.\n1번이 보통 메인, 2번 이후는 해설/영어 등.", L"播放多音轨视频前\n显示音轨选择界面。\n通常1为主音轨，2及以后为解说/英语等。", L"إظهار اختيار مسار الصوت قبل فيديو متعدد المسارات.\nالمسار 1 عادة الرئيسي؛ 2+ للتعليق/الإنجليزية.", L"Показать выбор аудiodорожки перед видео с несколькими дорожками.\n1 — обычно основная; 2+ — комментарий/английский.", L"Audiospur-Auswahl vor Mehrspur-Video.\nSpur 1 meist Haupt; 2+ Kommentar/Englisch.", L"Mostrar escolha de faixa antes de video multi-audio.\nFaixa 1 = principal; 2+ = comentario/ingles.", L"Audiospoorkeuze voor multi-audio video.\nSpoor 1 meestal hoofd; 2+ commentaar/Engels.", L"Wybor sciezki przed wideo wielosciezkowym.\n1 = glowna; 2+ = komentarz/angielski.", L"Coklu sesli videoda calmadan once\nses akisi secimini goster.\n1 genelde ana; 2+ yorum/Ingilizce."));
	m_tooltip.AddTool(GetDlgItem(IDC_CHECK49), LL14(L"対応しているkpiを24bit(ハイレゾ)で再生します。\n通常は16bitですが、まれに対応しているものがあります。\n音割れについては考慮されていないため、spcなど倍率を上げないといけないものは気をつけて下さい。", L"Play supported kpi at 24bit.\nUsually 16bit; some support 24bit.\nClipping not considered for spc etc.", L"Lire les kpi compatibles en 24 bits.\nSouvent 16 bits ; clipping non gere pour spc etc.", L"Riproduci kpi supportati a 24 bit.\nDi solito 16 bit; clipping non gestito per spc ecc.", L"Reproducir kpi compatibles a 24 bits.\nSuele ser 16 bits; sin anti-clipping para spc etc.", L"지원 kpi를 24bit(하이레z)로 재생.\n보통 16bit; spc 등 클리핑 미고려 주의.", L"以24bit播放支持的kpi。\n通常16bit；spc等未防削波请注意。", L"تشغيل kpi المدعوم بـ 24 بت.\nعادة 16 بت؛ بدون منع قص لـ spc.", L"Воспроизводить kpi в 24 бит.\nОбычно 16 бит; для spc клиппинг не учтен.", L"Kpi in 24 Bit abspielen.\nMeist 16 Bit; kein Clipping-Schutz fur spc.", L"Reproduzir kpi suportado em 24 bits.\nGeralmente 16 bits; sem anti-clipping para spc.", L"Ondersteunde kpi op 24 bit.\nMeestal 16 bit; geen clipping-bescherming voor spc.", L"Odtwarzaj kpi w 24 bit.\nZwykle 16 bit; brak ochrony przed clippingiem spc.", L"Desteklenen kpi'yi 24 bit cal.\nGenelde 16 bit; spc icin kirpma korunmaz."));
	m_tooltip.AddTool(GetDlgItem(IDC_CHECK51), LL14(L"対応しているkpiを32bit(ハイレゾ)で再生します。\n通常は16bitですが、まれに対応しているものがあります。\n音割れについては考慮されていないため、spcなど倍率を上げないといけないものは気をつけて下さい。", L"Play supported kpi at 32bit.\nUsually 16bit; some support 32bit.\nClipping not considered for spc etc.", L"Lire les kpi compatibles en 32 bits.\nSouvent 16 bits ; clipping non gere pour spc etc.", L"Riproduci kpi supportati a 32 bit.\nDi solito 16 bit; clipping non gestito per spc ecc.", L"Reproducir kpi compatibles a 32 bits.\nSuele ser 16 bits; sin anti-clipping para spc etc.", L"지원 kpi를 32bit(하이레z)로 재생.\n보통 16bit; spc 등 클리핑 미고려 주의.", L"以32bit播放支持的kpi。\n通常16bit；spc等未防削波请注意。", L"تشغيل kpi المدعوم بـ 32 بت.\nعادة 16 بت؛ بدون منع قص لـ spc.", L"Воспроизводить kpi в 32 бит.\nОбычно 16 бит; для spc клиппинг не учтен.", L"Kpi in 32 Bit abspielen.\nMeist 16 Bit; kein Clipping-Schutz fur spc.", L"Reproduzir kpi suportado em 32 bits.\nGeralmente 16 bits; sem anti-clipping para spc.", L"Ondersteunde kpi op 32 bit.\nMeestal 16 bit; geen clipping-bescherming voor spc.", L"Odtwarzaj kpi w 32 bit.\nZwykle 16 bit; brak ochrony przed clippingiem spc.", L"Desteklenen kpi'yi 32 bit cal.\nGenelde 16 bit; spc icin kirpma korunmaz."));
	...
	m_tooltip.AddTool(GetDlgItem(IDC_BUTTON1), LL14(L"碧の軌跡用のt_bgm._dtを設定します。", L"Set t_bgm._dt for Ao no Kiseki.", L"Definir t_bgm._dt pour Ao no Kiseki.", L"Imposta t_bgm._dt per Ao no Kiseki.", L"Establecer t_bgm._dt para Ao no Kiseki.", L"碧の軌跡용 t_bgm._dt 설정.", L"设置碧之轨迹的 t_bgm._dt。", L"تعيين t_bgm._dt لـ Ao no Kiseki.", L"Задать t_bgm._dt для Ao no Kiseki.", L"t_bgm._dt fur Ao no Kiseki festlegen.", L"Definir t_bgm._dt para Ao no Kiseki.", L"t_bgm._dt instellen voor Ao no Kiseki.", L"Ustaw t_bgm._dt dla Ao no Kiseki.", L"Ao no Kiseki icin t_bgm._dt ayarla"));

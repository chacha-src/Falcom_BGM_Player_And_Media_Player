...
static void MpRemoteHandleRequest(SOCKET s)
{
	...
	if (strstr(line, "GET /overlay")) {
		...
		CStringA utf8((LPCSTR)CW2A(html, CP_UTF8));
		CStringA hdr;
...
void MpOnVideoExtract(CMediaPlayerDlg* mpDlg)
{
	...
	if (!MpExtractVideoAudioToWav(item.fol, wavPath, &err)) {
		owner->MessageBox(err.IsEmpty()
			? CString(LL14(L"抽出に失敗しました。", L"Extract failed.", L"Echec extraction.", L"Estrazione fallita.", L"Extraccion fallida.",
...
BEGIN_MESSAGE_MAP(CMpDjPadDlg, CCustomBlurDialogBase)
	...
	ON_WM_CONTEXTMENU()
	ON_WM_RBUTTONUP()
	ON_BN_CLICKED(IDC_DJPAD_PITCH_UP, OnPitchUp)
	ON_BN_CLICKED(IDC_DJPAD_PITCH_DN, OnPitchDn)
	ON_BN_CLICKED(IDC_DJPAD_TEMPO_UP, OnTempoUp)
	ON_BN_CLICKED(IDC_DJPAD_TEMPO_DN, OnTempoDn)
	ON_BN_CLICKED(IDC_DJPAD_PITCH_RST, OnPitchRst)
	ON_BN_CLICKED(IDC_DJPAD_TEMPO_RST, OnTempoRst)
	ON_BN_CLICKED(IDC_DJPAD_VOCAL, OnVocal)
	ON_BN_CLICKED(IDC_DJPAD_VOCAL_DN, OnVocalDn)
	ON_BN_CLICKED(IDC_DJPAD_VOCAL_RST, OnVocalRst)
	ON_BN_CLICKED(IDC_DJPAD_MS_NARROW, OnMsNarrow)
	ON_BN_CLICKED(IDC_DJPAD_MS_WIDE, OnMsWide)
	ON_BN_CLICKED(IDC_DJPAD_MS_RST, OnMsRst)
	ON_BN_CLICKED(IDC_DJPAD_PLAY, OnPlay)
	ON_BN_CLICKED(IDC_DJPAD_PAUSE, OnPause)
	ON_BN_CLICKED(IDC_DJPAD_STOP, OnStop)
	ON_BN_CLICKED(IDC_DJPAD_CUE, OnCueBtn)
	ON_BN_CLICKED(IDC_DJPAD_PREV, OnPrev)
	ON_BN_CLICKED(IDC_DJPAD_NEXT, OnNext)
	ON_BN_CLICKED(IDC_DJPAD_BEAT_BK, OnBeatBk)
	ON_BN_CLICKED(IDC_DJPAD_BEAT_FW, OnBeatFw)
	ON_BN_CLICKED(IDC_DJPAD_BPM_DET, OnBpmDet)
	ON_CONTROL_RANGE(BN_CLICKED, IDC_DJPAD_CUE1, IDC_DJPAD_CUE8, OnCuePad)
	ON_BN_CLICKED(IDC_DJPAD_CUESET, OnCueSet)
	ON_BN_CLICKED(IDC_DJPAD_CUECLR, OnCueClr)
	ON_BN_CLICKED(IDC_DJPAD_ABA, OnAbA)
	ON_BN_CLICKED(IDC_DJPAD_ABB, OnAbB)
	ON_BN_CLICKED(IDC_DJPAD_ABCLR, OnAbClr)
	ON_BN_CLICKED(IDC_DJPAD_LOOP1, OnLoop1)
	ON_BN_CLICKED(IDC_DJPAD_LOOP2, OnLoop2)
	ON_BN_CLICKED(IDC_DJPAD_LOOP4, OnLoop4)
	ON_BN_CLICKED(IDC_DJPAD_LOOP8, OnLoop8)
	ON_BN_CLICKED(IDC_DJPAD_KILL_L, OnKillL)
	ON_BN_CLICKED(IDC_DJPAD_KILL_M, OnKillM)
	ON_BN_CLICKED(IDC_DJPAD_KILL_H, OnKillH)
	ON_CBN_SELCHANGE(IDC_DJPAD_MICDEV, OnMicDev)
	ON_BN_CLICKED(IDC_DJPAD_MICDEV_REFRESH, OnMicDevRefresh)
	ON_CBN_SELCHANGE(IDC_DJPAD_LOOPDEV, OnLoopDev)
END_MESSAGE_MAP()
...
BEGIN_MESSAGE_MAP(CMpBpmDlg, CCustomBlurDialogBase)
	ON_WM_SIZE()
	ON_WM_CONTEXTMENU()
	ON_BN_CLICKED(IDC_MP_BPM_CAND1, OnCand1)
	ON_BN_CLICKED(IDC_MP_BPM_CAND2, OnCand2)
	ON_BN_CLICKED(IDC_MP_BPM_CAND3, OnCand3)
	ON_BN_CLICKED(IDC_MP_BPM_REMEAS, OnRemeasure)
	ON_BN_CLICKED(IDC_MP_BPM_ABORT, OnAbort)
	ON_BN_CLICKED(IDC_MP_BPM_CLOSE, OnCloseBtn)
	ON_BN_CLICKED(IDC_MP_BPM_HELP, OnHelp)
END_MESSAGE_MAP()
...
BEGIN_MESSAGE_MAP(CMpRemoteDlg, CCustomBlurDialogBase)
	ON_WM_DESTROY()
	ON_EN_CHANGE(IDC_REMOTE_PORT, OnEnChangePort)
	ON_BN_CLICKED(IDC_REMOTE_OPEN, OnOpenBrowser)
	ON_WM_CONTEXTMENU()
	ON_WM_RBUTTONUP()
END_MESSAGE_MAP()

...
BEGIN_MESSAGE_MAP(CMpSmartPlaylistDlg, CCustomBlurDialogBase)
	ON_BN_CLICKED(IDC_MSM_ADD, OnBnClickedAdd)
	ON_BN_CLICKED(IDC_MSM_UPDATE, OnBnClickedUpdate)
	ON_BN_CLICKED(IDC_MSM_DEL, OnBnClickedDel)
	ON_BN_CLICKED(IDC_MSM_APPLY, OnBnClickedApply)
	ON_BN_CLICKED(IDC_MSM_CLOSE, OnBnClickedClose)
	ON_NOTIFY(LVN_ITEMCHANGED, IDC_MSM_LIST, OnLvnItemchangedList)
	ON_WM_CLOSE()
	ON_WM_CTLCOLOR()
END_MESSAGE_MAP()

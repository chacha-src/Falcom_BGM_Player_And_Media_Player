...
void CPlayList::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_STATICido, m_grpIdo);
	DDX_Control(pDX, IDC_STATICken, m_grpKen);
}
...
BOOL CPlayList::OnInitDialog()
{
	...
	m_grpIdo.SetWindowText(LL14(L"ファイル移動", L"File move", L"Deplacer fichier", L"Sposta file", L"Mover archivo", L"파일 이동", L"文件移动", L"نقل الملف", L"Переместить файл", L"Datei verschieben", L"Mover arquivo", L"Bestand verplaatsen", L"Przenie? plik", L"Dosya ta??"));
	m_grpKen.SetWindowText(LL14(L"あいまい検索", L"Fuzzy search", L"Recherche floue", L"Ricerca fuzzy", L"Busqueda difusa", L"퍼지 검색", L"模糊搜索", L"بحث غامض", L"Нечеткий поиск", L"Fuzzy-Suche", L"Pesquisa fuzzy", L"Fuzzy zoeken", L"Wyszukiwanie rozmyte", L"Bulan?k arama"));

...
void S98DEVICE_OPL2::SetReg(Uint32 addr, Uint32 data)
{
	bEnable = true;
	if (ifp) {
		::YM3812Write(ifp, 0, addr & 0xFF);
		::YM3812Write(ifp, 1, data);
	}
	FmMonShadowSetOplMode(1);
	FmMonShadowWriteOplReg(addr & 0x1FF, data);
}
...
{
	bEnable = true;
	const int port = (addr & 0x100) ? 2 : 0;
	if (ifp) {
		::YMF262Write(ifp, port, addr & 0xFF);
		::YMF262Write(ifp, port + 1, data);
	}

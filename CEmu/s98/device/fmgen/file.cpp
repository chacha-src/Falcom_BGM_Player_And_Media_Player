...
bool FileIO::Open(const _TCHAR* filename, uint flg)
{
	...
	hfile = CreateFile((LPCTSTR)filename, access, share, 0, creation, 0, 0);
...
bool FileIO::CreateNew(_TCHAR* filename)
{
	Close();
...
bool FileIO::Reopen(uint flg)
{
	...
	hfile = CreateFile((LPCTSTR)path, access, share, 0, creation, 0, 0);

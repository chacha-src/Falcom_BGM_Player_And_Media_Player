...
INLINE UINT8 RDMEM(offs_t addr) {
	...
	return h6280_bus_read(TRANSLATED(addr));
}
...
INLINE void WRMEM(offs_t addr, UINT8 data) {
	CHECK_VDC_VCE_PENALTY(addr);
	h6280_bus_write(TRANSLATED(addr),data);
}
 ...
 ***************************************************************/
#define RDMEMZ(addr) 											\
	h6280_bus_read( (h6280.mmr[1] << 13) | ((addr)&0x1fff));

/***************************************************************
 *  WRMEMZ   write memory - zero page
 ***************************************************************/
#define WRMEMZ(addr,data) 										\
	h6280_bus_write( (h6280.mmr[1] << 13) | ((addr)&0x1fff),data);

/***************************************************************
 *  RDMEMW   read word from memory
 ***************************************************************/
#define RDMEMW(addr)											\
	h6280_bus_read(TRANSLATED(addr)) \
| ( h6280_bus_read(TRANSLATED(addr+1)) << 8 )
...
#define RDZPWORD(addr)											\
	((addr&0xff)==0xff) ?										\
		h6280_bus_read( (h6280.mmr[1] << 13) | ((addr)&0x1fff))				\
		+(h6280_bus_read( (h6280.mmr[1] << 13) | ((addr-0xff)&0x1fff))<<8) : \
		h6280_bus_read( (h6280.mmr[1] << 13) | ((addr)&0x1fff))				\
		+(h6280_bus_read( (h6280.mmr[1] << 13) | ((addr+1)&0x1fff))<<8)


/***************************************************************
 * push a register onto the stack
 ***************************************************************/
#define PUSH(Rg) h6280_bus_write( (h6280.mmr[1] << 13) | h6280.sp.d,Rg); S--

/***************************************************************
 * pull a register from the stack
 ***************************************************************/
#define PULL(Rg) S++; Rg = h6280_bus_read( (h6280.mmr[1] << 13) | h6280.sp.d)

/***************************************************************
 *  RDOP    read an opcode
 ***************************************************************/
#define RDOP()													\
	h6280_bus_read(TRANSLATED(PCW))

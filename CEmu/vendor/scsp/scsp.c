...
static struct _SCSP *AllocedSCSP = 0;
...
static void SCSP_w16(struct _SCSP *SCSP,unsigned int addr,unsigned short val)
{
	...
	else if(addr<0x600)
	{
		...
		/* CEmu: the sound stack is 64 words at 0x600-0x67F, but the decode
		   runs to 0x6FF and indexed straight off the base — a write above
		   0x67F ran past RINGBUF and over Int68kCB, so the next IRQ check
		   called a garbage pointer (Model 2A doa/fvipers/manxtt crashed
		   here). Wrap to the array like the hardware mirror does. */
		SCSP->RINGBUF[((addr-0x600)/2)&63]=val;
	else
	{
		//DSP
		if(addr<0x780)	//COEF
			*((unsigned short *) (SCSP->DSP.COEF+(addr-0x700)/2))=val;
			...
			/* MADRS is 32 words at 0x780-0x7BF; 0x7C0-0x7FF mirrors it. */
			*((unsigned short *) (SCSP->DSP.MADRS+(((addr-0x780)/2)&31)))=val;
		else if(addr<0xC00)
		{
			*((unsigned short *) (SCSP->DSP.MPRO+(addr-0x800)/2))=val;

			SCSPDSP_UpdateProgram(&SCSP->DSP);
			...
			if(addr==0xBF0)
			{
				...
				if (SCSP->DSP.LastStep && !SCSP->dspstart)
				{
					/* DSP stabilizer(dirty hack) */
#if 1
					int i = SCSP->DSP.LastStep << 10;
					SCSP->dspstart = 1;
					while (--i)
...
static unsigned short SCSP_r16(struct _SCSP *SCSP, unsigned int addr)
{
	...
	else if(addr<0x600)
	{
		...
		v=SCSP->RINGBUF[((addr-0x600)/2)&63];
	return v;
}
	...
	for(s=0;s<nsamples;++s)
	{
		...
		for(sl=0;sl<32;++sl)
		{
			...
			if(SCSP->Slots[sl].active)
			{
...
#else
				if (dwChannelMute & (1 << sl))
					SCSPDSP_SetSample(&SCSP->DSP,0,ISEL(slot),IMXL(slot));
				else if(!SDIR(slot))
				{
					Enc=((TL(slot))<<0x0)|((IMXL(slot))<<0xd);
					SCSPDSP_SetSample(&SCSP->DSP,(sample*SCSP->LPANTABLE[Enc])>>(SHIFT-2),ISEL(slot),IMXL(slot));				
				...
				{
					Enc=(0<<0x0)|((IMXL(slot))<<0xd);
					SCSPDSP_SetSample(&SCSP->DSP,(sample*SCSP->LPANTABLE[Enc])>>(SHIFT-2),ISEL(slot),IMXL(slot));				
					Enc=(0<<0x0)|((DIPAN(slot))<<0x8)|((DISDL(slot))<<0xd);
					{
						smpl+=(sample*SCSP->LPANTABLE[Enc])>>SHIFT;
						smpr+=(sample*SCSP->RPANTABLE[Enc])>>SHIFT;
					}
...
/* ---- CEmu additions -------------------------------------------------------
   The FM monitor needs per-slot key state and playback rate, which live in the
   private slot struct, and the wave staging path needs the RAM pointer. These
   accessors only read state — the synthesis path is untouched. */
int CEmuScspSlotActive(int s)
{
	struct _SCSP *SCSP = AllocedSCSP;
	struct _SLOT *slot;
	if (!SCSP || s < 0 || s >= 32) return 0;
	slot = SCSP->Slots + s;
	return (slot->active && slot->EG.state != RELEASE) ? 1 : 0;
}

/* Playback rate relative to the native sample rate (1.0 = unity). SCSP builds
   its phase step from a 4-bit signed octave plus a 10-bit fraction. */
double CEmuScspSlotRate(int s)
{
	...
	int octave;
	double ratio;
	if (!SCSP || s < 0 || s >= 32) return 0.0;
	slot = SCSP->Slots + s;
	octave = (OCT(slot) ^ 8) - 8;
	ratio = 1.0 + (double)FNS(slot) / 1024.0;
	while (octave > 0) { ratio *= 2.0; octave--; }
	while (octave < 0) { ratio *= 0.5; octave++; }
	return ratio;
}
...
void scsp_term(void)
{
	...
	if (SCSP)
	{
		SCSPDSP_Term(&SCSP->DSP);
		if (SCSP->buffertmpl) free(SCSP->buffertmpl);
		if (SCSP->buffertmpr) free(SCSP->buffertmpr);
		free(SCSP);
		AllocedSCSP = 0;
	}
}

...
#define DUMP(v)	fprintf(f," " #v ": %04X",v);
...
#endif
		//INPUTS RW
		/* CEmu: IRA is 6 bits but only 0x00-0x31 are defined; Model 2 von2
		   runs programs with the top codes, which used to trip the assert
		   (and left INPUTS uninitialised in a release build). Undefined
		   selectors read as zero. */
		if(IRA<=0x1f)
			INPUTS=DSP->MEMS[IRA];
		else if(IRA<=0x2F)
		...
		//if(0)
		{
			...
			ADDR+=DSP->RBP<<12;
			/* CEmu: RBP is 6 bits and RBL up to 64K words, so the ring
			   buffer can be programmed past the end of the 512KB sound RAM
			   (Model 2 von2 did, and the DSP read faulted). Wrap into the
			   RAM the way the chip's address lines do. */
			if(DSP->SCSPRAM_LENGTH)
				ADDR&=DSP->SCSPRAM_LENGTH-1;
...
void SCSPDSP_Term(struct _SCSPDSP *DSP)
{
}

void SCSPDSP_UpdateProgram(struct _SCSPDSP *DSP)
{
}

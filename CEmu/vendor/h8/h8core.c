/* Auto-generated detached H8/3002 core for CEmu (from MAME 0.149 h83002). */
#include "h8core.h"
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
...
enum {
	H8_IRQ0 = 0, H8_IRQ1, H8_IRQ2, H8_IRQ3, H8_IRQ4, H8_IRQ5
};
...
struct h83xx_state {
	...
	UINT16 h8TCNT[5];
	UINT8 h8TSTR;
	UINT8 ddrs[H8_MAX_PORTS], drs[H8_MAX_PORTS];
	void* bus_ctx;
	H8ReadFn bus_read;
	H8WriteFn bus_write;
	UINT32 irq_count;
	UINT32 itu_subcycles[5];
	UINT8 int_ram[0x200];
};

struct H8Cpu { h83xx_state s; };

static void h8_check_irqs(h83xx_state *h8);
void h8_3002_InterruptRequest(h83xx_state *h8, UINT8 source, UINT8 state);

static void h8_itu_reset(h83xx_state *h8)
{
	int i;
	h8->h8TSTR = 0;
	memset(h8->per_regs, 0, sizeof(h8->per_regs));
	for (i = 0; i < 5; i++) {
		UINT32 base = 0x64 + i * 10;
		h8->h8TCNT[i] = 0;
		h8->itu_subcycles[i] = 0;
		h8->per_regs[base + 2] = 0xf8; /* TIER: unused bits read high */
		h8->per_regs[base + 3] = 0xf8; /* TSR: unused bits read high */
		h8->per_regs[base + 6] = 0xff; /* GRA reset value */
		h8->per_regs[base + 7] = 0xff;
		h8->per_regs[base + 8] = 0xff; /* GRB reset value */
		h8->per_regs[base + 9] = 0xff;
	}
}
static void h8_itu_init(h83xx_state *h8) { h8_itu_reset(h8); }
static void h8_itu_tick(h83xx_state *h8, int cycles)
{
	int ch;
	if (!h8->h8TSTR) return;
	for (ch = 0; ch < 5; ch++) {
		UINT32 base, ticks, divider;
		UINT16 gra, grb, count;
		UINT8 tcr, tier;
		if (!(h8->h8TSTR & (1 << ch))) continue;
		base = 0x64 + ch * 10;
		tcr = h8->per_regs[base];
		/* H8/300H internal clocks are phi, phi/2, phi/4 and phi/8.
		   TCR values 4-7 select external pins, which this detached core
		   cannot advance from CPU cycles. */
		if ((tcr & 7) >= 4) continue;
		divider = 1u << (tcr & 7);
		h8->itu_subcycles[ch] += (UINT32)cycles;
		ticks = h8->itu_subcycles[ch] / divider;
		h8->itu_subcycles[ch] %= divider;
		if (!ticks) continue;

		tier = h8->per_regs[base + 2];
		gra = (UINT16)((h8->per_regs[base + 6] << 8) | h8->per_regs[base + 7]);
		grb = (UINT16)((h8->per_regs[base + 8] << 8) | h8->per_regs[base + 9]);
		count = h8->h8TCNT[ch];
		while (ticks--) {
			int clear = 0;
			/* MAME's H8H timer schedules TGR value + 1 clocks.  Testing
			   the current count before increment gives that same period
			   (GRA=0 fires after one timer clock). */
			if (count == gra) {
				h8->per_regs[base + 3] |= 1;
				if (tier & 1)
					h8_3002_InterruptRequest(h8, (UINT8)(24 + 4 * ch), 1);
				if ((tcr & 0x60) == 0x20) clear = 1;
			}
			if (count == grb) {
				h8->per_regs[base + 3] |= 2;
				if (tier & 2)
					h8_3002_InterruptRequest(h8, (UINT8)(25 + 4 * ch), 1);
				if ((tcr & 0x60) == 0x40) clear = 1;
			}
			if (count == 0xffff) {
				h8->per_regs[base + 3] |= 4;
				if (tier & 4)
					h8_3002_InterruptRequest(h8, (UINT8)(26 + 4 * ch), 1);
			}
			count = clear ? 0 : (UINT16)(count + 1);
		}
		h8->h8TCNT[ch] = count;
		h8->per_regs[base + 4] = (UINT8)(count >> 8);
		h8->per_regs[base + 5] = (UINT8)count;
	}
}

static UINT8 h8_register_read8(h83xx_state *h8, UINT32 address)
{
	UINT32 reg = address & 0xff;
	UINT8 val;
	if (reg >= sizeof(h8->per_regs)) return 0;
	if (reg == 0x60) return h8->h8TSTR;
	val = h8->per_regs[reg];
	/* SCI0/SCI1 SSR @ FFB4/FFBC — classic MAME returns TDRE|RDRF|TEND. */
	if (reg == 0xb4 || reg == 0xbc)
		val = (UINT8)(val | 0xc4);
	/* ND-1 ncv1 waits on FFFFE8 bit7 (DTC/status) in a tight poll loop;
	   without a ready stub the MCU never reaches the song mailbox. */
	if (reg == 0xe8)
		val = (UINT8)(val | 0x80);
	/* FFFFE0-FFFEF: treat unread status lanes as ready (open-bus high). */
	if (reg >= 0xe0 && reg <= 0xef && val == 0)
		val = 0xff;
	return val;
}
static void h8_register_write8(h83xx_state *h8, UINT32 address, UINT8 val)
{
	UINT32 reg = address & 0xff;
	int ch;
	if (reg >= sizeof(h8->per_regs)) return;
	if (reg == 0x60) {
		h8->h8TSTR = val;
		h8->per_regs[reg] = val;
		return;
	...
	for (ch = 0; ch < 5; ch++) {
		UINT32 base = 0x64 + ch * 10;
		if (reg == base + 3) {
			/* H8H status flags clear when software writes the bit as zero. */
			h8->per_regs[reg] = (UINT8)(0xf8 | (h8->per_regs[reg] & val & 7));
			return;
		}
		if (reg == base + 4 || reg == base + 5) {
			h8->per_regs[reg] = val;
			h8->h8TCNT[ch] = (UINT16)((h8->per_regs[base + 4] << 8)
				| h8->per_regs[base + 5]);
			return;
		}
	}
...
static UINT8 h8_bus_read8(h83xx_state *h8, UINT32 address)
{
	address &= 0xffffff;
	if (address >= 0xfffd10 && address <= 0xffff0f)
		return h8->int_ram[address - 0xfffd10];
	if (address >= 0xffff10)
		return h8_register_read8(h8, address);
	if (h8->bus_read) return h8->bus_read(h8->bus_ctx, address);
	return 0xff;
}
static void h8_bus_write8(h83xx_state *h8, UINT32 address, UINT8 data)
{
	address &= 0xffffff;
	if (address >= 0xfffd10 && address <= 0xffff0f) {
		h8->int_ram[address - 0xfffd10] = data;
		return;
	}
	if (address >= 0xffff10) {
		h8_register_write8(h8, address, data);
		return;
	}
	if (h8->bus_write) h8->bus_write(h8->bus_ctx, address, data);
}
static UINT16 h8_bus_read16(h83xx_state *h8, UINT32 address)
{
	UINT16 hi = h8_bus_read8(h8, address);
	UINT16 lo = h8_bus_read8(h8, address + 1);
	return (UINT16)((hi << 8) | lo);
}
static void h8_bus_write16(h83xx_state *h8, UINT32 address, UINT16 data)
{
	h8_bus_write8(h8, address, (UINT8)(data >> 8));
	h8_bus_write8(h8, address + 1, (UINT8)data);
}


#define H8_SP   (7)
#define h8_mem_read8(x)       h8_bus_read8(h8, (UINT32)(x))
#define h8_mem_write8(x, y)   h8_bus_write8(h8, (UINT32)(x), (UINT8)(y))
#define h8_mem_read16(z, x)   h8_bus_read16(h8, (UINT32)(x))
#define h8_mem_write16(z, x, y) h8_bus_write16(h8, (UINT32)(x), (UINT16)(y))
#define h8_readop16(x, y)     h8_bus_read16(h8, (UINT32)(y))

#define H8_IFETCH_TIMING(x) h8->cyccnt -= (x) * 4;
#define H8_BRANCH_TIMING(x) h8->cyccnt -= (x) * 4;
#define H8_STACK_TIMING(x)  h8->cyccnt -= (x) * 4;
#define H8_BYTE_TIMING(x, adr)  h8->cyccnt -= (x) * 4;
#define H8_WORD_TIMING(x, adr)  h8->cyccnt -= (x) * 4;
...
{
	UINT32 result = h8_bus_read16(h8, address) << 16;
	return result | h8_bus_read16(h8, address + 2);
}

INLINE void h8_mem_write32(h83xx_state *h8, offs_t address, UINT32 data)
{
	h8_bus_write16(h8, address, (UINT16)(data >> 16));
	h8_bus_write16(h8, address + 2, (UINT16)data);
}
...
static void h8_execute_inner(h83xx_state *h8, int *cycles)
{
	UINT16 opcode=0;
...
H8Cpu* H8Create(void)
{
	H8Cpu* cpu = (H8Cpu*)calloc(1, sizeof(H8Cpu));
	if (!cpu) return NULL;
	cpu->s.h8iflag = 1;
	cpu->s.mode_8bit = 0;
	h8_itu_init(&cpu->s);
...
void H8Destroy(H8Cpu* cpu) { free(cpu); }
...
void H8Reset(H8Cpu* cpu)
{
	...
	if (!cpu) return;
	h8 = &cpu->s;
	memset(h8->regs, 0, sizeof(h8->regs));
	h8->h8err = 0;
	h8->h8iflag = 1;
	h8->incheckirqs = 0;
	h8->exr = 0;
	h8->has_exr = 0;
	h8->has_h8speriphs = 0;
	h8->irq_req[0] = h8->irq_req[1] = h8->irq_req[2] = 0;
	h8_itu_reset(h8);
	/* Classic MAME: PC = *(u32*)0 — Namco bootstrap lives at that address. */
...
int H8Execute(H8Cpu* cpu, int cycles)
{
	...
	left = cycles;
	h8_itu_tick(&cpu->s, cycles);
	h8_execute_inner(&cpu->s, &left);
	return cycles - (left > 0 ? left : 0);
}
void H8SetInputLine(H8Cpu* cpu, int line, int state)
{
	if (!cpu) return;
	if (line >= 0 && line <= 5)
		h8_3002_InterruptRequest(&cpu->s, (UINT8)(12 + line), state ? 1 : 0);
	else if (line == H8_LINE_NMI)
		h8_3002_InterruptRequest(&cpu->s, 7, state ? 1 : 0);
	if (state) cpu->s.irq_count++;
}
uint32_t H8Pc(const H8Cpu* cpu) { return cpu ? cpu->s.pc : 0; }
uint32_t H8Sp(const H8Cpu* cpu) { return cpu ? cpu->s.regs[7] : 0; }
uint32_t H8Err(const H8Cpu* cpu) { return cpu ? cpu->s.h8err : 0; }
uint32_t H8IrqCount(const H8Cpu* cpu) { return cpu ? cpu->s.irq_count : 0; }

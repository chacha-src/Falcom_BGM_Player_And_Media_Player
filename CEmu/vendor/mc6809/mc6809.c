...
int mc6809_step(mc6809__t *cpu)
{
  ...
  switch(inst)
  {
    ...
    case 0x3B:
         /* RTI: E comes from the pulled CC (FIRQ stacked E=0 → CC+PC only;
            IRQ stacked E=1 → full register restore). Using pre-pull E
            misaligned the stack after FIRQ and left CC.F stuck. */
         cpu->cycles += 5;
         mc6809_bytetocc(cpu,(*cpu->read)(cpu,cpu->S.w++,false));
         if (cpu->cc.e)
         {

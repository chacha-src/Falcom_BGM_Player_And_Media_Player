...
static uint8_t m6800_do_debug_read(struct m6800 *cpu, uint16_t addr);
...
static int m6800_execute_one(struct m6800 *cpu)
{
 ...
 /* An invalid instruction we don't yet model — soft-trap, don't abort host. */
 if (cpu->debug)
 fprintf(stderr, "illegal instruction %02X at %04X\n",
 opcode, fetch_pc);
 cpu->pc = (uint16_t)(fetch_pc + 1);
...
{
	if (cpu->read) return cpu->read(cpu, addr);
	return 0xff;
}
void m6800_write(struct m6800 *cpu, uint16_t addr, uint8_t val)
{
	if (cpu->write) cpu->write(cpu, addr, val);
}
uint8_t m6800_port_input(struct m6800 *cpu, int port)
{
	if (cpu->port_in) return cpu->port_in(cpu, port);
	return 0xff;
}
void m6800_port_output(struct m6800 *cpu, int port)
{
	uint8_t v = (port == 1) ? cpu->p1dr : cpu->p2dr;
	if (cpu->port_out) cpu->port_out(cpu, port, v);
}
void m6800_sci_change(struct m6800 *cpu) { (void)cpu; }
void m6800_tx_byte(struct m6800 *cpu, uint8_t val) { (void)cpu; (void)val; }
uint8_t m6800_debug_read(struct m6800 *cpu, uint16_t addr) { return m6800_read(cpu, addr); }
static uint8_t m6800_do_debug_read(struct m6800 *cpu, uint16_t addr) { return m6800_debug_read(cpu, addr); }

...
* Doesn't implement the R register.
* EI delays maskable IRQs by one instruction (irqDelay); see Ay_Cpu.h.
...
#define READ_WORD( addr )           ( (uint16_t)ay_cpu_read( this, (unsigned)(uint16_t)(addr) ) | \
	( (uint16_t)ay_cpu_read( this, (unsigned)(uint16_t)((addr) + 1) ) << 8 ) )
#define WRITE_WORD( addr, data )    do { \
	const uint16_t _wa = (uint16_t)(addr); \
	const uint16_t _wd = (uint16_t)(data); \
	ay_cpu_write( this, (unsigned)_wa, (int)(_wd & 0xff) ); \
	ay_cpu_write( this, (unsigned)(uint16_t)(_wa + 1), (int)(_wd >> 8) ); \
} while (0)
...
// Misc
	...
	case 0xFB: // EI
		r.iff1 = 1;
		r.iff2 = 1;
		irqDelay = 1; /* accept maskable IRQ only after the next insn */
		goto loop;

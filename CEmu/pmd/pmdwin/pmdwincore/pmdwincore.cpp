...
int WINAPI PMDWIN::music_load(TCHAR *filename)
{
	...
	extern PMDWIN* g_pmdwinFmMonActive;
	g_pmdwinFmMonActive = this;
...
void WINAPI PMDWIN::getpcmdata(int16_t *buf, int nsamples)
{
	...
	/* FM monitor: live dump（KPI 非依存。公式 kbpmd のまま使える） */
	{
		extern void PmdFmMonFlushAfterRender(PMDWIN* self, uint32_t sampleRate, uint64_t curSample);
		static uint64_t s_cur = 0;
		s_cur += (uint64_t)(uint32_t)nsamples;
		PmdFmMonFlushAfterRender(this, (uint32_t)open_work.rate, s_cur);
	}
}
...
void PMDWIN::FmMonSnapshot(uint8_t regs[0x200], uint8_t writeBits[64],
...
{
	if (!opna) return;
	opna->FmMonSnapshot(regs, writeBits, keyOnFm, keyOnHit, ssgOn, ssgHit,
		rhythmKey, rhythmPulse, rhythmHit, seq);
}

void PMDWIN::FmMonAckPulse(void)
{
	if (!opna) return;
	opna->FmMonAckPulse();
}

long long PMDWIN::FmMonAdpcmBDeltaFromFmRegdata(void)
{
	if (!opna) return 0;
	return opna->FmMonAdpcmBDeltaFromFmRegdata();
}

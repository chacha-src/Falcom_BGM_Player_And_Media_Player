...
BEGIN_MESSAGE_MAP(CPianoRoll, CCustomBlurDialogExBase)
    ...
    ON_WM_DESTROY()
    ON_WM_CONTEXTMENU()
    ON_BN_CLICKED(IDC_PR_HELP, OnBnClickedHelp)
    ON_COMMAND(ID_HELP_SHOWSHEET, OnBnClickedHelp)
    ON_COMMAND_RANGE(IDM_ROLL_SPEED_BASE, IDM_ROLL_SPEED_BASE + ROLL_SPEED_COUNT - 1, OnRollSpeedCmd)
    ON_COMMAND(IDM_ROLL_FREEZE, OnToggleFreeze)
    ON_COMMAND(IDM_ROLL_CLEAR, OnClearDisplay)
    ON_COMMAND(IDM_ROLL_LEGEND, OnToggleExprLegend)
    ON_COMMAND(IDM_ROLL_EXPR, OnToggleExprMarks)
    ON_COMMAND(IDM_ROLL_METER, OnToggleLevelMeter)
    ON_COMMAND(IDM_ROLL_TOPMOST, OnToggleAlwaysOnTop)
    ON_COMMAND(IDM_ROLL_REATTACK, OnToggleReattackDetect)
    ON_COMMAND(IDM_ROLL_IMPULSE, OnToggleImpulsiveGhost)
    ON_COMMAND(IDM_ROLL_HARM_GHOST, OnToggleHarmonicGhost)
    ON_COMMAND(IDM_ROLL_HARM_PROF, OnToggleHarmonicProfile)
    ON_COMMAND(IDM_ROLL_TUNE, OnOpenTuneDialog)
    ON_COMMAND_RANGE(IDM_ROLL_VIEW_BASE, IDM_ROLL_VIEW_BASE + IDM_ROLL_VIEW_COUNT - 1, OnViewModeCmd)
    ON_COMMAND(IDM_ROLL_CAM_RESET, OnCamResetCmd)
    ON_COMMAND_RANGE(IDM_ROLL_KEYS_BASE, IDM_ROLL_KEYS_BASE + IDM_ROLL_KEYS_COUNT - 1, OnKeyRangeCmd)
    ON_COMMAND(IDM_ROLL_NOTENAME, OnToggleNoteNames)
    ON_COMMAND(IDM_ROLL_CAPTURE_MIDI, OnToggleCaptureMidi)
    ON_COMMAND(IDM_ROLL_CAPTURE_MUSICXML, OnToggleCaptureMusicXml)
    ON_COMMAND(IDM_ROLL_CHORD_PANEL, OnToggleChordPanel)
    ON_COMMAND(IDM_ROLL_LOOPBACK_SCORE, OnToggleLoopbackScore)
    ON_WM_LBUTTONDOWN()
    ON_WM_MOUSEMOVE()
    ON_WM_LBUTTONUP()
    ON_WM_SETCURSOR()
...
void CPianoRoll::OnContextMenu(CWnd* /*pWnd*/, CPoint point)
{
    ...
    menu.AddSeparator();
    CCustomPopupMenu* subDisp = menu.AddSubMenu(
        ...
        LL14(L"ノート名／凡例／表現記号／メーター／最前面などの表示切替。", L"Toggle note names, legend, expression marks, meter, always-on-top.", L"Basculer noms, legende, symboles, metre, premier plan.", L"Attiva/disattiva nomi, legenda, simboli, metro, primo piano.", L"Alternar nombres, leyenda, simbolos, medidor, siempre visible.", L"음이름/범례/표현 기호/미터/항상 위 표시 전환.", L"切换音名/图例/奏法记号/电平表/置顶等显示。", L"تبديل أسماء النغمات والدليل والرموز والمقياس والوضع الأمامي.", L"Переключать имена нот, легенду, знаки, метр, поверх всех.", L"Notennamen, Legende, Ausdruck, Pegel, Vordergrund umschalten.", L"Alternar nomes, legenda, simbolos, medidor, sempre no topo.", L"Schakel notennamen, legenda, tekens, meter, altijd-boven om.", L"Przelacz nazwy nut, legende, znaki, miernik, zawsze na wierzchu.", L"Nota adi, aciklama, ifade, olcer, her zaman ustte gecis."));
    if (subDisp) {
        subDisp->AddCheck(IDM_ROLL_NOTENAME,
        ...
        subDisp->AddCheck(IDM_ROLL_LEGEND,
        ...
        subDisp->AddCheck(IDM_ROLL_EXPR,
        ...
        subDisp->AddCheck(IDM_ROLL_METER,
        ...
        subDisp->AddCheck(IDM_ROLL_TOPMOST,
    ...
    menu.AddSeparator();
    CCustomPopupMenu* subOps = menu.AddSubMenu(
        ...
        LL14(L"検出パラメータ調整、フリーズ、表示クリア。", L"Detection tuning, freeze, and clear display.", L"Reglage detection, gel, effacer l'affichage.", L"Regolazione rilevamento, congelamento, cancella.", L"Ajuste de deteccion, congelar y borrar.", L"검출 파라미터 조정, 프리즈, 표시 지우기.", L"检测参数调整、冻结、清除显示。", L"ضبط الكشف والتجميد ومسح العرض.", L"Настройка обнаружения, заморозка, очистка.", L"Erkennungsparameter, Einfrieren, Anzeige leeren.", L"Ajuste de deteccao, congelar e limpar.", L"Detectieparameters, bevriezen en wissen.", L"Parametry wykrywania, zamrozenie, wyczysc.", L"Algilama ayari, dondur ve temizle."));
    if (subOps) {
        subOps->AddCommand(IDM_ROLL_TUNE,
        ...
        subOps->AddCheck(IDM_ROLL_FREEZE,
        ...
        subOps->AddCommand(IDM_ROLL_CLEAR,
    ...
    menu.AddSeparator();
    CCustomPopupMenu* subOpen = menu.AddSubMenu(
        LL14(L"開く", L"Open", L"Ouvrir", L"Apri", L"Abrir", L"열기", L"打开", L"فتح", L"Открыть", L"Offnen", L"Abrir", L"Openen", L"Otworz", L"Ac"),
        LL14(L"イコライザ／アナライザ／操作ガイドを開きます。", L"Open equalizer, analyzer, or the operation guide.", L"Ouvrir egaliseur, analyseur ou le guide.", L"Apri equalizzatore, analizzatore o la guida.", L"Abrir ecualizador, analizador o la guia.", L"이퀄라이저/분석기/조작 가이드를 엽니다.", L"打开均衡器/分析器/操作指南。", L"فتح المعادل أو المحلل أو دليل التشغيل.", L"Открыть эквалайзер, анализатор или руководство.", L"Equalizer, Analyzer oder Bedienungsanleitung offnen.", L"Abrir equalizador, analisador ou o guia.", L"Open equalizer, analyzer of handleiding.", L"Otworz equalizer, analizator lub przewodnik.", L"Equalizer, analizor veya islem kilavuzunu ac."));
    if (subOpen) {
        subOpen->AddCommand(ID_MP_OPEN_EQ,
        ...
        subOpen->AddCommand(ID_MP_OPEN_ANALYZER,

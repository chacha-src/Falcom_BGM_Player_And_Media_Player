...
BEGIN_MESSAGE_MAP(CProToolsDlg, CCustomBlurDialogBase)
	ON_BN_CLICKED(IDC_PRO_APPLY, OnBnClickedApply)
	ON_BN_CLICKED(IDC_PRO_CUEADD, OnBnClickedCueAdd)
	ON_BN_CLICKED(IDC_PRO_CUEDEL, OnBnClickedCueDel)
	ON_BN_CLICKED(IDC_PRO_CUEJUMP, OnBnClickedCueJump)
	ON_BN_CLICKED(IDC_PRO_LOOPIN_BTN, OnBnClickedLoopIn)
	ON_BN_CLICKED(IDC_PRO_LOOPOUT_BTN, OnBnClickedLoopOut)
	ON_BN_CLICKED(IDC_PRO_WRITETAG, OnBnClickedWriteTag)
	ON_BN_CLICKED(IDC_PT_HELP, OnBnClickedHelp)
	ON_BN_CLICKED(IDC_PRO_GAPLESS, OnBnClickedLiveFlag)
	ON_BN_CLICKED(IDC_PRO_MSMONO, OnBnClickedLiveFlag)
	ON_BN_CLICKED(IDC_PRO_CORR, OnBnClickedLiveFlag)
	ON_BN_CLICKED(IDC_PRO_EXPLIMIT, OnBnClickedLiveFlag)
	ON_BN_CLICKED(IDC_PRO_EXPTP, OnBnClickedLiveFlag)
	ON_CBN_SELCHANGE(IDC_PRO_RGMODE, OnCbnSelchangeRgMode)
	ON_WM_HSCROLL()
	ON_EN_KILLFOCUS(IDC_PRO_RGTARGET, OnEnKillfocusLiveEdit)
	ON_EN_KILLFOCUS(IDC_PRO_EXPCEIL, OnEnKillfocusLiveEdit)
	ON_WM_LBUTTONDOWN()
	ON_WM_PAINT()
	ON_WM_ERASEBKGND()
	ON_WM_SIZE()
	...
	ON_WM_DESTROY()
	ON_WM_CONTEXTMENU()
	ON_COMMAND(IDM_PT_VOCAL_CANCEL, OnVocalPresetCancel)
	ON_COMMAND(IDM_PT_VOCAL_NEUTRAL, OnVocalPresetNeutral)
	ON_COMMAND(IDM_PT_VOCAL_EMPH, OnVocalPresetEmphasize)
	ON_COMMAND(IDM_PT_MS_NARROW, OnMsPresetNarrow)
	ON_COMMAND(IDM_PT_MS_WIDE, OnMsPresetWide)
	ON_COMMAND(IDM_PT_MS_MONO, OnMsPresetMono)
	ON_COMMAND(IDM_PT_MS_RESET, OnMsPresetReset)
END_MESSAGE_MAP()
...
void CProToolsDlg::OnContextMenu(CWnd* pWnd, CPoint point)
{
	...
	menu.AddSeparator();
	{
		CCustomPopupMenu* playSub = menu.AddSubMenu(
			LL14(L"再生/書き出し", L"Playback/Export", L"Lecture/Export", L"Riproduzione/Export", L"Reproduccion/Exportacion",
				L"재생/내보내기", L"播放/导出", L"تشغيل/تصدير", L"Воспроизведение/Экспорт", L"Wiedergabe/Export",
				L"Reproducao/Exportacao", L"Afspeelen/Export", L"Odtwarzanie/Eksport", L"Calma/Disa aktarma"),
			LL14(L"ギャップレス、相関メーター、書き出しリミット／True Peak。", L"Gapless, correlation meter, export limit / True Peak.", L"Sans silence, correlometre, limites d'export / True Peak.", L"Senza gap, correlazione, limiti export / True Peak.", L"Sin pausa, correlacion, limites de exportacion / True Peak.", L"갭리스, 상관 미터, 내보내기 리밋/True Peak.", L"无缝、相关表、导出限制/True Peak。", L"بدون فجوة ومقياس الترابط وحدود التصدير/True Peak.", L"Без паузы, корреляция, лимиты экспорта / True Peak.", L"Luckenlos, Korrelation, Exportlimit / True Peak.", L"Sem intervalo, correlacao, limites de exportacao / True Peak.", L"Naadloos, correlatie, exportlimiet / True Peak.", L"Bez przerw, korelacja, limit eksportu / True Peak.", L"Bosluksuz, korelasyon, disa aktarma limiti / True Peak."));
		if (playSub) {
			playSub->AddCheck(IDM_PT_GAPLESS,
			...
			playSub->AddCheck(IDM_PT_EXPLIMIT,

...
BEGIN_MESSAGE_MAP(CSasamiFmVoiceDlg, CCustomBlurDialogExBase)
	...
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_BN_CLICKED(IDC_SASAMI_FV_PREVIEW, OnBnClickedPreview)
	ON_BN_CLICKED(IDC_SASAMI_FV_OK, OnBnClickedOk)
	ON_BN_CLICKED(IDC_SASAMI_FV_CANCEL, OnBnClickedCancel)
	ON_CBN_SELCHANGE(IDC_SASAMI_FV_PRESET, OnCbnPreset)
	ON_BN_CLICKED(IDC_SASAMI_FV_APPLY, OnBnClickedApplyHex)
END_MESSAGE_MAP()

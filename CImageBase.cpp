...
IMPLEMENT_DYNAMIC(CImageBase, CCustomBlurDialogExBase)

CImageBase::CImageBase(CWnd* pParent /*=nullptr*/)
	: CCustomBlurDialogExBase(IDD_IMAGEBASE, pParent)
{
	oya = NULL;
}
...
BEGIN_MESSAGE_MAP(CImageBase, CCustomBlurDialogExBase)
	ON_WM_LBUTTONUP()
	ON_WM_LBUTTONDOWN()
	ON_WM_MOUSEMOVE()
	ON_WM_PAINT()
...
BOOL CImageBase::DestroyWindow()
{
	...
	BOOL rr = CCustomBlurDialogExBase::DestroyWindow();
	delete this;
	return rr;
}
...
void CImageBase::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	GetWindowRect(a.o);
	a.p = &point;
	if (point.x <= 0 || point.y <= 0) return CCustomBlurDialogExBase::OnLButtonDown(nFlags, point);
...
void CImageBase::OnPaint()
{
	CPaintDC dc(this); // device context for painting
					   ...
					   // 描画メッセージで CCustomBlurDialogExBase::OnPaint() を呼び出さないでください。
	COSVersion os;
	os.GetVersionString();
	if (os.in.dwMajorVersion == 10 && os.in.dwBuildNumber >= 22000){}
	else {
...
LRESULT CImageBase::OnNcHitTest(CPoint point)
{
	...
	return CCustomBlurDialogExBase::OnNcHitTest(point);
}

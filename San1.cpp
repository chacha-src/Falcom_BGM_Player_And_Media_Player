...
BOOL CSan1::OnInitDialog()
{
	...
	m_msg.SetWindowText(LL14(L"動画(★印)を再生するにはBinkの環境が必要です\\nreadme.txtを読んで導入してください。", L"Bink runtime required for video (★) playback.\\nSee readme.txt for setup.", L"Runtime Bink requis pour la lecture vidéo (★).\\nConsultez readme.txt pour l'installation.", L"Runtime Bink necessario per la riproduzione video (★).\\nVedi readme.txt per l'installazione.", L"Se requiere runtime Bink para reproducir vídeo (★).\\nConsulte readme.txt para la instalación.", L"동영상(★) 재생에는 Bink 런타임이 필요합니다\\n설치 방법은 readme.txt를 참조하세요.", L"播放视频(★)需要 Bink 运行环境\\n请参阅 readme.txt 进行安装。", L"مطلوب Bink runtime لتشغيل الفيديو (★).\\nراجع readme.txt للإعداد.", L"Для воспроизведения видео (★) нужна среда Bink.\\nСм. readme.txt для установки.", L"Bink-Laufzeit für Video(★)-Wiedergabe erforderlich.\\nSiehe readme.txt zur Einrichtung.", L"Runtime Bink necessário para reproduzir vídeo (★).\\nConsulte readme.txt para instalação.", L"Bink-runtime vereist voor video(★)-afspelen.\\nZie readme.txt voor installatie.", L"Do odtwarzania wideo (★) wymagany jest runtime Bink.\\nZobacz readme.txt w celu instalacji.", L"Video (★) oynatmak için Bink runtime gerekir.\\nKurulum için readme.txt dosyasına bakın."));

...
BEGIN_MESSAGE_MAP(CAnalyzerDlg, CCustomBlurDialogExBase)
	...
	ON_WM_ERASEBKGND()
	ON_WM_CONTEXTMENU()
	ON_BN_CLICKED(IDC_AN_HELP, OnBnClickedHelp)
	...
	ON_WM_SETCURSOR()
	ON_WM_MOUSEWHEEL()
	ON_COMMAND(IDM_SPEC_OVERLAY, OnSpecLayoutOverlay)
	ON_COMMAND(IDM_SPEC_SPLIT_V, OnSpecLayoutSplitV)
	ON_COMMAND(IDM_SPEC_SPLIT_H, OnSpecLayoutSplitH)
	ON_COMMAND(IDM_SPEC_GRID4, OnSpecLayoutGrid4)
	ON_COMMAND(IDM_SPEC_GRID8, OnSpecLayoutGrid8)
	ON_COMMAND(IDM_STYLE_FILL, OnSpecStyleFill)
	ON_COMMAND(IDM_STYLE_LINE, OnSpecStyleLine)
	ON_COMMAND(IDM_STYLE_BARS, OnSpecStyleBars)
	ON_COMMAND(IDM_STYLE_CUBASE, OnSpecStyleCubase)
	ON_COMMAND(IDM_STYLE_SPAN, OnSpecStyleSpan)
	ON_COMMAND(IDM_STYLE_ABLETON, OnSpecStyleAbleton)
	ON_COMMAND(IDM_STYLE_FABFILTER, OnSpecStyleFabFilter)
	ON_COMMAND_RANGE(IDM_WAVE_SPEED_BASE, IDM_WAVE_SPEED_BASE + CAnalyzerDlg::WAVE_SPEED_COUNT - 1, OnWaveSpeedCmd)
	ON_COMMAND(IDM_PEAK_HOLD, OnTogglePeakHold)
	ON_COMMAND(IDM_EQ_OVERLAY, OnToggleEqOverlay)
	ON_COMMAND(IDM_FREEZE, OnToggleFreeze)
	ON_COMMAND(IDM_RESET_PEAK, OnResetPeakHold)
	ON_COMMAND(IDM_LEVEL_METER, OnToggleLevelMeter)
	ON_COMMAND(IDM_ALWAYS_ON_TOP, OnToggleAlwaysOnTop)
	ON_COMMAND(IDM_CLEAR_DISPLAY, OnClearDisplay)
	ON_COMMAND(IDM_COPY_HOVER, OnCopyHoverReadout)
	ON_COMMAND(IDM_COPY_PEAK, OnCopyPeakFreq)
	ON_COMMAND(IDM_COPY_LEVELS, OnCopyLevels)
	ON_COMMAND(IDM_WAVE_SCROLL, OnWaveModeScroll)
	ON_COMMAND(IDM_WAVE_TRIGGER, OnWaveModeTrigger)
	ON_COMMAND_RANGE(IDM_WAVE_ENVELOPE, IDM_WAVE_PHOSPHOR, OnWaveModeExtra)
	ON_COMMAND(IDM_LOWER_SPECTRUM, OnLowerModeSpectrum)
	ON_COMMAND(IDM_LOWER_WATERFALL, OnLowerModeWaterfall)
	ON_COMMAND(IDM_LOWER_PHASE, OnLowerModePhase)
	ON_COMMAND_RANGE(IDM_LOWER_VECTOR, IDM_LOWER_PHASEFREQ, OnLowerModeExtra)
	ON_COMMAND(IDM_SPEC_DIFF, OnToggleSpecDiff)
	ON_COMMAND(IDM_SPEC_SNAP, OnCaptureSpecSnap)
	ON_COMMAND(IDM_SPEC_SNAP_CLEAR, OnClearSpecSnap)
	ON_COMMAND(IDM_ZOOM_FULL, OnFreqZoomFull)
	ON_COMMAND(IDM_ZOOM_LOW, OnFreqZoomLow)
	ON_COMMAND(IDM_ZOOM_MID, OnFreqZoomMid)
	ON_COMMAND(IDM_ZOOM_HIGH, OnFreqZoomHigh)
	ON_COMMAND(IDM_MARKER_ADD, OnMarkerAdd)
	ON_COMMAND(IDM_MARKER_REMOVE, OnMarkerRemoveNearest)
	ON_COMMAND(IDM_MARKER_CLEAR, OnMarkerClearAll)
	ON_COMMAND(IDM_CORR_METER, OnToggleCorrMeter)
	ON_COMMAND(IDM_SAVE_JACKET, OnSaveSpectrogramJacket)
	ON_COMMAND(IDM_MS_NARROW, OnMsPresetNarrow)
	ON_COMMAND(IDM_MS_WIDE, OnMsPresetWide)
	ON_COMMAND(IDM_MS_MONO, OnMsPresetMono)
	ON_COMMAND(IDM_MS_RESET, OnMsPresetReset)
	ON_COMMAND(IDM_MS_FROM_CORR, OnMsPresetFromCorr)
...
void CAnalyzerDlg::RedrawSpectrum(COLORREF bg)
{
	...
	if (m_lowerMode == LowerCorrFreq || m_lowerMode == LowerPhaseFreq) {
		...
		m_specDC.TextOut(plot.left + 2, (int)(std::max)(0L, plot.top - 13),
			isCorr
			? CString(LL14(L"帯域相関 (L/R)", L"Correlation vs frequency (L/R)", L"Correlation par frequence (L/R)", L"Correlazione per frequenza (L/R)",
			...
			: CString(LL14(L"帯域位相差 (L/R)", L"Phase vs frequency (L/R)", L"Phase par frequence (L/R)", L"Fase per frequenza (L/R)",
